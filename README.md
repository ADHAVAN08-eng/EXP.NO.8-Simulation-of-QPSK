...
Plot:-In-phase component-Quadrature component-Resultant waveform.
# PROGRAM
...
Number of QPSK symbols Symbol period (seconds) Sampling frequency (Hz) Time vector for one symbol Generate random bit sequence bits = np.random.randint(0, 2, num_symbols * 2) # Two bits per symbol

symbols = 2 * bits[0::2] + bits[1::2]

Initialize QPSK signal qpsk_signal = np.array([])

symbol_times = []

Define phase mapping for QPSK (Gray Coding) symbol_phases = {
...
Map bits to symbol (0 to 3) Generate the QPSK modulated signal for i, symbol in enumerate(symbols):
...
Full time vector t_total = np.arange(0, num_symbols * T, 1/fs)

Plotting the QPSK signal plt.figure(figsize=(14, 12))

In-phase component plt.subplot(3, 1, 1)
...
Quadrature component plt.subplot(3, 1, 2)
...
Resultant QPSK waveform plt.subplot(3, 1, 3)
...
plt.show()
# OUTPUT
 ![image](https://github.com/user-attachments/assets/8147320c-a148-4a6e-941b-fa35b8458a82)
